...
addon = xbmcaddon.Addon('plugin.video.live.frtele')
...
apply_fyle = os.path.join(home, 'apply_fyle')
...
if os.path.exists(apply_fyle)==True:
    SOURCES = open(apply_fyle).read()
else: SOURCES = []
...
def getSources():
        try:
            ...
            if os.path.exists(apply_fyle)==True:
                sources = json.loads(open(apply_fyle,"r").read())
...
def addSource(url=None):
        ...
        if os.path.exists(apply_fyle)==False:
            source_list = []
...
def createM3uForDash(url,useragent=None):
    ...
    apply_fyle = os.path.join(profile, 'testfile.m3u')
    str+='\n'
    SaveToFile(apply_fyle,str)
